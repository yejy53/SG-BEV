...
class Neck_Sate(BaseModule):
    ...
    def forward(self, feature_maps):
        ...
        c4 = feature_maps[3] # 1/32
        c3 = feature_maps[2] # 1/16
        c2 = feature_maps[1] # 1/8

        # Apply linear layers
        c4_transformed = self.linear_c4(c4).permute(0, 2, 1).reshape(batch_size, -1, c4.shape[2], c4.shape[3]).contiguous() # 统一通道数
        ...
        c4_resized = F.interpolate(c4_transformed, size=c2.size()[2:], mode='bilinear', align_corners=False) # 插值提升分辨率
        ...
        fused_feature_map = self.linear_fuse(torch.cat([c4_resized, c3_resized, c2_resized], dim=1)) # MLP结合通道

        # 转置卷积回复分辨率
        upsampled_feature_map = self.conv_transpose1(fused_feature_map) 
